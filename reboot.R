...
parser_reg$add_argument("-P", "--pcentfilter", type = "double", dest = "pf",  metavar = '', default = 0.3,
                        help = "Percentage of correlated feature pairs allowed in each iteration (double). Default: 0.3")
parser_reg$add_argument("-T", "--ty", type = "character", dest = "ty",  metavar = '', default = "gene",
                        help = 'Type of transcriptome data: gene or transcript (character). Default: gene')
parser_reg$add_argument("-V", "--varfilter", type = "double", dest = "var",  metavar = '', default = 0.01,
                        help = 'Minimum normalized variance (0-1) required for each feature among samples (double). Default: 0.01')
parser_reg$add_argument("-F", "--force", dest = "force", action = "store_true", default = FALSE,
...
parser_sur$add_argument("-M", "--multivariate", dest = "type", action = "store_true", default = FALSE,
...
parser_sur$add_argument("-R", "--roc", dest = "roc_curve", action = "store_true", default = FALSE,
                        help = 'If molecular score should be categorized according to a ROC curve instead of the median, choose -R')
parser_sur$add_argument("-V", "--varfilter", type = "double", dest = "var",  metavar = '', default = 0.01,
                        help = 'Minimum normalized variance (0-1) required for follow up time (double). Default: 0.01')
parser_sur$add_argument("-F", "--force", dest = "force", action = "store_true", default = FALSE,
                        help = "Choose -F to bypass OS and OS.time filters")
#####################################################################################################################
...
parser_all$add_argument("-I", "--filein", type = "character", dest = "fname",  metavar = '',
                        help = "Input file name. Tab separated values (tsv) file containing expression and survival parameters")
parser_all$add_argument("-O", "--outprefix", type = "character", dest = "out",  metavar = '', default = "reboot",
                        help = 'Output file prefix. Default: reboot')
parser_all$add_argument("-M", "--multivariate", dest = "type", action = "store_true", default = FALSE,
                        help = 'If clinical variables should be included, choose -M. This option is tied with -C option')
parser_all$add_argument("-C", "--clinical", type = "character", dest = "clin_file",  metavar = '', default = "",
                        help = 'Tab separated values (tsv) file containing binary categorical variables only. Required if -M option is chosen')
parser_all$add_argument("-R", "--roc", dest = "roc_curve", action = "store_true", default = FALSE,
                        help = 'If molecular score should be categorized according to a ROC curve instead of the median, choose -R')
parser_all$add_argument("-B","--bootstrap", type = "integer", dest = "boot_iter",  metavar = '', default = 1,
                        help = "Number of iterations for bootstrap simulation (int). Default: 1")
parser_all$add_argument("-G", "--groupsize", type = "integer", dest = "nel",  metavar = '', default = 10,
                        help = "Number of genes/transcripts for each bootstrap simulation (int). Default: 10")
parser_all$add_argument("-P", "--pcentfilter", type = "double", dest = "pf",  metavar = '', default = 0.3,
                        help = "Percentage of correlated feature pairs allowed in each iteration (double). Default: 0.3")
parser_all$add_argument("-T", "--ty", type = "character", dest = "ty",  metavar = '', default = "gene",
                        help = 'Type of transcriptome data: gene or transcript (character). Default: gene')
parser_all$add_argument("-V", "--varfilter", type = "double", dest = "var",  metavar = '', default = 0.01,
                        help = 'Minimum normalized variance (0-1) required for each feature and follow up time (double). Default: 0.01')
parser_all$add_argument("-F", "--force", dest = "force", action = "store_true", default = FALSE,
                        help = "Choose -F to bypass OS and OS.time filters")
#####################################################################################################################
